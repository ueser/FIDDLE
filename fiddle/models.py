...
class NNscaffold(object):
    ...
    def __init__(self,
        ...
        for track_name in self.architecture['Inputs']:
            ...
            self.representations[track_name] = self.tracks[
                track_name].representation
        ...
        self.dropout = tf.placeholder(
            tf.float32)  # initializing data type input for dropout
        self.keep_prob_input = tf.placeholder(
            tf.float32)  # initializing data type input for keep_prob_input
        # Used for modality-wise dropout. Equivalent to batch_size for training, test size for testing
        self.inp_size = tf.placeholder(
            tf.int32)  # initializing data type input for keep_prob_input
        self._combine_representations(
            mode=
            'convolution')  # combines representations into convolutional layer
