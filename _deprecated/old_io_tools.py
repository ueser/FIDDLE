...
class MultiThreadRunner(object):
    ...
    def __init__(self, train_h5_handle, inputs, outputs, architecture):
        """
        Args:
            train_h5_handle: hdf5 handle -or pointer-
            inputs: a dict object that holds tf placeholders for each input track
            outputs: a dict object that holds tf placeholders for each output track
            architecture: a nested dict object that holds architecture of the network
        """
